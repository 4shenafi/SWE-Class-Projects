...
public :
	...
	AIRLINE () {
		id = 0;
		locations [0] = (char *)"ADDIS ABABA" ;
		locations [1] = (char *)"HAWASSA" ; 
		locations [2] = (char *)"BAHIR DAR" ;
	   	locations [3] = (char *)"DIRE DAWA" ;
	   	locations [4] = (char *)"MEKELLE" ;
		locations [5] = (char *)"ARBA MINCH" ;
	}
	...
	while (true) {
        ...
        if (cin >> number) {
            ...
            if (number >= 1 && number <= 8) {
                break;  // Exit the loop, as a valid number was entered
            } else {
                cout << "Error: Number out of range. Please try again." << endl;
...
mm_re:
	...
	cin >> mm;
	if (mm == 13 && dd > 5) {
		cout << "pagume , 2016 has only 5 Days . Please enter the date again\n";
		goto dd_re;
	}
	if (mm < 0 ||  mm > 13) {
		cout << "Months are in range [1 - 13] only , Please make a correct choice \n";
		goto mm_re;
	}
yy_re:
	cout << "Year [YYYY] : ";
...
while (true) {
        ...
        } else if (phone[0] != '0' || (phone[1] != '9' && phone[1] != '7')) {
            cout << "You entered an invalid phone number. Please Try Again.\n";
        } else {
            int i = 0;
            int count = 0;
            while (phone[i] >= '0' && phone[i] <= '9' && phone[i] != '\0') {
                count++;
                i++;
            }

            if (count == 10) {
                break;  // Exit the loop as a valid phone number was entered
            } else {
                cout << "You entered an invalid phone number. Please Try Again.\n";
            }
        }
	...
	while (true) {
        if (cin >> userFlightNo) {
            ...
            if (userFlightNo >= 1) {
                ...
                cout << "Your userFlightNo doesn't match as per the Flight Standards [>=1]. Please enter your valid Flight Number\n";
            }
        } else {
            cout << "Error: Invalid input. Please enter a valid userFlightNo." <<endl;
            cin.clear();  // Clear the error flag
            cin.ignore(numeric_limits<streamsize>::max(), '\n');  // Discard invalid input
        }
    }
...
{
	cout.setf(ios::left);
	cout <<setw(10)<<getBookingId()<<setw(15)<<getName()<<setw(12)<<getDate()<<setw(15)<<getUserFlightNo()<<setw(15)<<getPhone()<<setw(20)<<getAddress()<<setw(20)<<getEmail()<<setw(12)<<getTravelClass()<<setw(12)<<getPassport()<<setw(8)<<getSeatNo()<<endl;
}
	...
	while (true) {
		...
		if (cin >> temp) {
			...
			if (temp >= 1) {
				...
				cout << "Error: Flight Number out of range. Please try again." << endl;
				cin.clear();  // Clear the error flag
				cin.ignore(numeric_limits<streamsize>::max(), '\n');  // Discard invalid input
			}
		} else {
			cout << "Error: Invalid input. Please enter a valid Flight Number." <<endl;
			cin.clear();  // Clear the error flag
			cin.ignore(numeric_limits<streamsize>::max(), '\n');  // Discard invalid input
		}
	}
	...
	while (true) {
        ...
        if (cin >> book) {
            ...
            if (book >= 1 && book <= 3) {
				...
				cin.clear();  // Clear the error flag
                cin.ignore(numeric_limits<streamsize>::max(), '\n'); 
				goto stc; // Discard invalid input
            }
        } else {
            cout << "Error: Invalid input. Please enter a valid book number." <<endl;
            cin.clear();  // Clear the error flag
            cin.ignore(numeric_limits<streamsize>::max(), '\n');  
			goto stc;// Discard invalid input
        }
...
void AIRLINE::decSeatNum(char ch, int flNo) {
    fstream file("flights.dat", std::ios::in | std::ios::out | std::ios::binary);
    ...
    while (file.read(reinterpret_cast<char*>(&tmp), sizeof(tmp))) {
        if (flNo == tmp.getFlightNo()) {
            if (ch == 'E' && tmp.getSeatEconomy() > 0) {
                tmp.setSeatEconomy(tmp.getSeatEconomy() - 1);
            } else if (ch == 'B' && tmp.getSeatBusiness() > 0) {
                tmp.setSeatBusiness(tmp.getSeatBusiness() - 1);
            } else if (ch == 'P' && tmp.getSeatPremium() > 0) {
                tmp.setSeatPremium(tmp.getSeatPremium() - 1);
            }
            file.seekp(-static_cast<int>(sizeof(tmp)), std::ios::cur);
            file.write(reinterpret_cast<char*>(&tmp), sizeof(tmp));
            break;  // We found and updated the flight, no need to continue
        }
    }
...
int MENU()
{
    cout <<"\n\n\n\n\n\n\n\n\n";
	cout << "\t\t\t**********************************************************************\n";
	cout << "\t\t\t               Welcome to ETHIOPEAN AIRLINE SERVICES   \n";
	cout << "\t\t\t**********************************************************************\n";
...
void disFormat()
{
	cout <<endl;
	cout.setf(ios::left);
	cout<<setw(10)<<"Book. ID"<<setw(15)<<"Name"<<setw(12)<<"Date"<<setw(15)<<"Flight Id"<<setw(15)<<"Phone"<<setw(20)<<"Address"<<setw(20)<<"Email"<<setw(12)<<"Class"<<setw(12)<<"Passport"<<setw(8)<<" Seat "<<endl;
	cout<<setw(10)<<"--------"<<setw(15)<<"----"<<setw(12)<<"----"<<setw(15)<<"---------"<<setw(15)<<"-----"<<setw(20)<<"-----"<<setw(20)<<"-----"<<setw(12)<<"-----"<<setw(12)<<"--------"<<setw(8)<<"------"<<endl;
}
...
{
	char ch;
	cout << "Press 'r' to return to the Flights Portal Menu or 'q' to quit or any key to return to start Menu\n";
...
void password() {
    ...
    //char default_Password[15] = "group";
pass_re:
    cout << "Enter your Password : ";
    char id[15];
    cin >> id;

    if (strcmp(id , "groupthree") == 0) {
...
void AIRLINE :: printTicket() {
    cout << "\t\t\t###################################################################################\n";
    cout << "\t\t\t                           ETHIOPEAN AIRLINE SERVICES                            \n";
    cout << "\t\t\t---------------------------------------------------------------------------------  \n";
    cout << "\t\t\tClass : " <<getTravelClass() <<                                                  "\n";
    cout << "\t\t\t           Name                                               BookingID           \n";
    cout << "\t\t\t      --------------                                        ----------------       \n";
		...
		case 1:
			{
				...
				while (fl.eof() != 1)
				{
					if (strcmp(b.getDeparture() , tmp.getFlightOrigin()) == 0 && strcmp(b.getArrival() , tmp.getFlightDestination()) == 0 ) {
                        *p += 1;
						tmp.disFlightAll();
					}
					fl.read((char *)&tmp, sizeof(tmp));
			...
			flight_menu:
				...
				while (true) {
					...
					if (cin >> fl_choice) {
						...
						cout << "Error: Invalid input. Please enter a valid flght choice." <<endl;
						cin.clear();  // Clear the error flag
						cin.ignore(numeric_limits<streamsize>::max(), '\n');  // Discard invalid input
					}
				}
					...
					case 2:
						{
							...
							cout << "Enter the Flight No. for the flight that you want to remove: ";
							int flightNo;

							while (true) {
								if (cin >> flightNo && flightNo > 0) {
									break;  // Exit the loop, a valid number was entered
								} else {
									cout << "Error: Invalid input. Please enter a valid flight number." << endl;
									cin.clear();  // Clear the error flag
									cin.ignore(numeric_limits<streamsize>::max(), '\n');  // Discard invalid input
							...
							fstream flight_remove("flights.dat", ios::in | ios::out | ios::binary | ios::app);
							fstream flight_tmp("tmp.dat", ios::in | ios::out | ios::binary | ios::app);

							int flag = 0;
							while (flight_remove.read(reinterpret_cast<char*>(&flight), sizeof(flight))) {
								if (flightNo == flight.getFlightNo()) {
									...
									cout << "Press 'y' to confirm removing the flight from Operations: ";
									char ch;
									cin >> ch;

									if (ch == 'Y' || ch == 'y') {

										// Do not copy the flight to the temp file
									} else {
										flight_tmp.write(reinterpret_cast<char*>(&flight), sizeof(flight));
									}
								} else {
									flight_tmp.write(reinterpret_cast<char*>(&flight), sizeof(flight));
								}
							...
							if (flag == 0) {
								cout << "\nYou entered an invalid Flight Number\n";
							}

							if (remove("flights.dat") == 0 && rename("tmp.dat", "flights.dat") == 0) {
								cout << "Flight Successfully Removed from Operations.\n";
							} else {
								cout << "Error in removing or renaming the file.\n";
							}

							char choice = exitFlightFormat();
					...
					case 3:
						{
							...
							while (flight_fare.eof() != 1)
							{
								if (tmp == flight.getFlightNo())
								{
									...
									cout << "Premime : " << flight.getFare('P')<<endl;
					...
					case 4:
						{
							...
							flight_seat.close();
							flight_rewrite.close();
							remove("flights.dat");
							rename("flights_copy.dat","flights.dat");
				...
				while (true) 
					{
								...
								if (cin >> query) {
									...
									if(query>= 1) {
										...
										cout << "Error: booking ID out of range. Please try again." << endl;
										cin.clear();  // Clear the error flag
										cin.ignore(numeric_limits<streamsize>::max(), '\n');  // Discard invalid input
									}
								} else {
									cout << "Error: Invalid input. Please enter a valid booking ID." <<endl;
									cin.clear();  // Clear the error flag
									cin.ignore(numeric_limits<streamsize>::max(), '\n');  // Discard invalid input
								}
						}
				...
				while (true) 
					{
						if (cin >> choice) {
							...
							if(choice>= 1 || choice<=4) {
								...
								cout << "Error: choice out of range. Please try again." << endl;
								cin.clear();  // Clear the error flag
								cin.ignore(numeric_limits<streamsize>::max(), '\n');  // Discard invalid input
							}
						} else {
					...
					case 1:   // Editing the Passenger's Name
						{	
							...
							if (remove("booking.dat") == 0) {
								...
								if (rename("mod.dat","booking.dat") == 0) {
